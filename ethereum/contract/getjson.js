...
var metakey = "nchc";			//config encode name,timestamp,diploma name
	...
	function cutjson(){
		fs.readFile('./test.json','utf-8', (err, data) => {
			...
			fs.writeFile("./testtmp.json",JSON.stringify(json), function(err) {			//save json data into test2.json
				if(err) {
			...
			upload += "./testtmp.json";				//where store json file(tmp file)
			...
			cmd.get(
				upload,											//cmd to upload file
					...
					url =url+encdata+"&dat="+enccut+"\" a.jpg";
					console.log(url);
					cmd.get(
						url,
						function(err,data,stderr){
							console.log("qrcode : ",data);
							var deletejson = "rm -f ./testtmp.json";
