...
//solidity 0.4.16 new tools: pure added
...
contract test{
   ...
   function init(bytes20 decode,bytes20[] xoraddr){
        ...
        for(uint j = 0 ;j<xoraddr.length;j++){
            Diploma[i].relate.push(address(xoraddr[j]));
            Diploma[i].verified.push(false);
        }
        Diploma[i].CNT = 0;
   }
   ...
   function xorint(bytes20 num) pure returns(uint){
        bytes20 str = bytes20(sha3("idolmaster"));
        address con =this;
        return uint(str^bytes20(con)^num);
    }
    
    //sha3(nchc)^relate^contract used in getting init
    //odd,even has different encode mode(sha3(nchc)or sha3(ncnc1))
    function xorrelate(uint num,bytes20 relate) pure returns(address ans){
         address con = this;
         bytes20 str;
         if(num == 0){
   ...
   function verify(bytes20 numhex,bytes20 ihex,bytes32 hash,uint8 v,bytes32 xorr,bytes32 xors){
    ...
    //unxor sign(r,s)
    function xorsign(bytes32 xorr) pure returns(bytes32 r){
        address con = this;
